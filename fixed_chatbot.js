...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function sendMessage() {
    ...
    // Process message with delay for realistic effect
    setTimeout(() => {
  ...
  function processMessage(message) {
    ...
    if (pageResponses.default_message) {
      ...
      response = `I'm here to help with any questions about Black Armor AI drone security solutions. What specific information are you looking for?`;
    }
  ...
  function executeCommand(command, label) {
    // Use provided label or look it up from commands
    const userMessage = label || pageCommands.find(cmd => cmd.command === command)?.label || command;
    ...
    const response = pageResponses[command] || `I'm sorry, I don't have information about "${command}" yet.`;
    ...
    setTimeout(() => {
      ...
      // Re-display command chips after response
      setTimeout(() => {
        displayCommandChips(pageCommands);
      }, 500);
    }, 1500);
